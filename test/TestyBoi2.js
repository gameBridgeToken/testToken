...
contract('TestyBoi2', function(accounts) {
    ...
    it('inits contract with the correct values', function() {
        return TestyBoi2.deployed().then(function(instance) {
            tokenInstance = instance;
            return tokenInstance.name();
        }).then(function(name){
            assert.equal(name, 'TestyBoi2', 'has correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'tBoi2', 'has correct symbol');
        })
    });

    it('allocates total supply on deploy', function() {
        return TestyBoi2.deployed().then(function(instance){
            tokenInstance = instance;
    ...
    it('handles delegated transfer of token ownership', function() {
        return TestyBoi2.deployed().then(function(instance){
            tokenInstance = instance;
            ...
            return tokenInstance.transferFrom(fromAccount, toAccount, 5000, { from: spendingAccount })
        }).then(assert.fail).catch(function(error){
